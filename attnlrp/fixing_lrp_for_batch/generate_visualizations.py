        ...
        for batch_idx, (data, target) in enumerate(tqdm(sample_loader)):
           ...
           # np.save(f"input_test_{batch_idx}.npy", data.data.cpu().numpy())
            ...
            if args.method    == 'custom_lrp':
                 ...
                 Res = lrp.generate_LRP(data, method="custom_lrp", cp_rule = args.cp_rule, index=index)
                 print(f"RES_shape: {Res.shape}")
                 Res = Res.reshape(data.shape[0],14, 14)
                 print(f"RES_shape: {Res.shape}")
            ...
            if args.method == 'rollout':
                ...
                #    count_correct+=1
                print(lrp.generate_LRP(data, start_layer=1, method="grad",  cp_rule = args.cp_rule, index=index).shape)
            ...
            for i in range(data.shape[0]):
              curr_map = Res[i,0,:,:]
              Res[i,0,:,:] = (curr_map - curr_map.min()) / (curr_map.max() - curr_map.min())


            #np.save(f"test_custom_{batch_idx}.npy", Res.data.cpu().numpy() )
            print("reached")
