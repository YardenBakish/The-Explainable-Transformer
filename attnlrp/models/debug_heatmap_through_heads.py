...
if __name__ == "__main__":
  ...
  for module in model.modules():
    if isinstance(module, Attention):
      ...
      end_idx = 1 * head_dim
      q_weight = module.qkv.weight.data[start_idx:end_idx]
      k_weight = module.qkv.weight.data[dim+start_idx:dim+end_idx]
      v_weight = module.qkv.weight.data[2*dim+start_idx:2*dim+end_idx]
      combined_weights = torch.cat([q_weight, k_weight, v_weight], dim=0)
      module.qkv_h1 = Linear(dim, head_dim * 3, bias=module.qkv.bias is not None).cuda()
      module.qkv_h1.weight.data = combined_weights
      # For head 2
      ...
      v_weight = module.qkv.weight.data[2*dim+start_idx:2*dim+end_idx]
      combined_weights = torch.cat([q_weight, k_weight, v_weight], dim=0)
      module.qkv_h2 = Linear(dim, head_dim * 3, bias=module.qkv.bias is not None).cuda()
      module.qkv_h2.weight.data = combined_weights
      # For head 3
      ...
      v_weight = module.qkv.weight.data[2*dim+start_idx:2*dim+end_idx]
      combined_weights = torch.cat([q_weight, k_weight, v_weight], dim=0)
      module.qkv_h3 = Linear(dim, head_dim * 3, bias=module.qkv.bias is not None).cuda()
      module.qkv_h3.weight.data = combined_weights
  model.eval()
