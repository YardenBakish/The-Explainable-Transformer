...
__all__ = ['forward_hook', 'Clone', 'Add', 'Cat', 'ReLU', 'GELU', 'Dropout', 'BatchNorm2d', 'Linear', 'MaxPool2d',
           ...
           'Sparsemax',
           'RepBN',
           'SiLU', 'WeightNormLinear', 'NormalizedLayerNorm' , 'NormalizedConv2d', 'LinearEpsilon']
...
class LinearEpsilon(nn.Linear):
    ...
    def forward(ctx, X, epsilon=1e-6):
        ...
        outputs = F.linear(X, self.weight, self.bias)
        ctx.save_for_backward(X, self.weight, outputs)
        ctx.epsilon = epsilon
