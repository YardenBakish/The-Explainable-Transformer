...
args1.exp_img_path = os.path.join(saver.results_dir, 'explain/img')
if not os.path.exists(args1.exp_img_path):
    os.makedirs(args1.exp_img_path)
args1.exp_np_path = os.path.join(saver.results_dir, 'explain/np')
if not os.path.exists(args1.exp_np_path):
    os.makedirs(args1.exp_np_path)
...
def eval_batch(image, labels, evaluator, index):
    ...
    if args1.method    == 'custom_lrp':
        Res1 = lrp1.generate_LRP(image.cuda(), method="custom_lrp", cp_rule = args1.cp_rule).reshape(14, 14).unsqueeze(0).unsqueeze(0) 
        Res2 = lrp2.generate_LRP(image.cuda(), method="custom_lrp", cp_rule = args2.cp_rule).reshape(14, 14).unsqueeze(0).unsqueeze(0) 
    ...
    elif args1.method == 'full_lrp':
        ResX = orig_lrp.generate_LRP(image.cuda(), method="full", cp_rule = args.cp_rule).reshape(batch_size, 1, 224, 224)
    ...
    #print(pixAcc1)
    print("love for the streets")
    #print(pixAcc1, pixAcc2)
    ...
    conc_imgs = concatenate_images_with_gaps([label_img, heatmap1, heatmap2, ])
    ...
    plt.imsave(f"/content/The-Explainable-Transformer/test_vis/{index}_{winner_pixAcc}_{winner_miou}_{winner_iou_fore}.png"  ,conc_imgs)
...
plt.figure()
plt.plot(rc, pr)
plt.savefig(os.path.join(saver.experiment_dir, 'PR_curve_{}.png'.format(args1.method)))
